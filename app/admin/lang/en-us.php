<?php
// +----------------------------------------------------------------------
// | ThinkCMF [ WE CAN DO IT MORE SIMPLE ]
// +----------------------------------------------------------------------
// | Copyright (c) 2013-2019 http://www.thinkcmf.com All rights reserved.
// +----------------------------------------------------------------------
// | Licensed ( http://www.apache.org/licenses/LICENSE-2.0 )
// +----------------------------------------------------------------------
// | Author: 老猫 <thinkcmf@126.com>
// +----------------------------------------------------------------------
return [
    'ADMIN_CENTER'            => 'Admin Center',
    'WELCOME_USER'            => 'Welcome, {:user_nickname}',
    'REFRESH_CURRENT_PAGE'    => 'Refresh Current Page',
    'CLOSE_TOP_MENU'    => 'Close Top menu',
    'WEBSITE_HOME_PAGE'       => 'Website Home Page',
    "SAVE"                    => 'Save',
    "CLOSE"                   => 'Close',
    "OPEN"                    => 'Open',
    "ENABLED"                 => 'Enabled',
    "DISABLED"                => 'Disabled',
    "DELETE"                  => "Delete",
    "RESTORE"                 => 'Restore',
    "DOWNLOAD"                => 'Download',
    "YES"                     => 'Yes',
    "NO"                      => 'No',
    "EDIT"                    => 'Edit',
    "ADD"                     => 'Add',
    "BACK"                    => 'Back',
    'LOGIN'                   => 'Login',
    'PASSWORD'                => "Password",
    'USERNAME_OR_EMAIL'       => 'Username or Email',
    'LOGIN_SUCCESS'           => "Login success!",
    'PASSWORD_NOT_RIGHT'      => "Password Error!",
    'PASSWORD_REQUIRED'       => "Password cannot be empty!",
    'CAPTCHA_REQUIRED'        => "Verification code cannot be empty!",
    'CAPTCHA_NOT_RIGHT'       => "Verification code error!",
    'USERNAME_NOT_EXIST'      => "Username does not exist!",
    'USERNAME_OR_EMAIL_EMPTY' => "Username or email address can't for empty!",
    'USE_DISABLED'            => "This user has been disabled!",
    'ENTER_VERIFY_CODE'       => 'Please enter the verification code!',
    'HOME'                    => 'Home',
    'LOADING'                 => 'Loading...',
    'LOGOUT'                  => 'Logout',
    "OK"                      => "Ok",
    "STATUS"                  => 'Status',
    "ACTIONS"                 => 'Actions',
    "SORT"                    => 'Sort',
    "DISPLAY"                 => 'Display',
    "HIDDEN"                  => 'Hidden',
    "HIDE"                    => 'Hide',
    'DELETE_CONFIRM_MESSAGE'  => 'Are you sure to delete?',
    'SETTING'                 => 'Setting',
    "ADD_SUCCESS"             => 'Add success!',
    "ADD_FAILED"              => 'Add failed!',
    "EDIT_SUCCESS"            => 'Save success！',
    "EDIT_FAILED"             => 'Save failed！',
    "NO_DATA"                 => 'No data!',
    "MOBILE"                  => 'Mobile',
    "CATEGORY_KEY_HELP_TEXT" => 'ALPHANUM or Underscores',
    // 订单模块
    "ORDER_INPUT" => 'input',
    "ACCOUNT_NUM" => '充值账号',

    // 系统模块 收款账户设置 gathering_account_setting
    "GATHERING_ACCOUNT_SETTING__CHANNELTYPE" => '收款渠道类别',
    "GATHERING_ACCOUNT_SETTING__CHANNELACCOUNTNUM" => '收款渠道账户',
    "GATHERING_ACCOUNT_SETTING__GRANDTOTALDEPOSITTIMES" => '累计存款次数',
    "GATHERING_ACCOUNT_SETTING__GRANDTOTALAMOUNTDEPOSITED" => '累计存款金额',
    "GATHERING_ACCOUNT_SETTING__CREATETIME" => '账户创建时间',
    "GATHERING_ACCOUNT_SETTING__USEDSTATUS" => '使用状态',
    "GATHERING_ACCOUNT_SETTING__ISACTIVE" => '是否启用',
    "GATHERING_ACCOUNT_SETTING__DESCRIPTION" => '备注',
    "GATHERING_ACCOUNT_SETTING__MODIFYTIME" => '最后存款时间',
    "GATHERING_ACCOUNT_SETTING__SALESID" => '销售人员ID',
];